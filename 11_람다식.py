...
# 잘못 입력 시 다시 입력받을 수 있도록 함
# 빠진 알파벳 입력했을 때는 게임 종료, 게임 플레이한 시간 출력
...
# startTime = time.time()
# randAlpha = random.choice(data)
# game_on = True
# count = 0
# while game_on :
#     try :
#         user_Alpha = input("알파벳을 입력해주세요.")
#         if not(user_Alpha in data) :
#             pirnt("다시 입력해 주세요.")
#             continue
#     except :
...
#         continue
#     count += 1   
#     if user_Alpha == randAlpha :
#         game_on = False


# endTime = time.time()
# print(f'경과시간 : {endTime-startTime}')
# print(f'{randAlpha}를 {count}횟수로 맞추셨습니다.')
...
endTime = time.time()
print(f'경과시간 : {endTime-startTime:.1f}')