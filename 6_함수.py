...
print(getweek(2023, 11, 2, True))
print(getweek(2023, 11, 2))
# %%

def jera(year, mon, day):
    if mon <= 2:
        mon += 12
        year -= 1
    a, b = divmod(year,100)
...
print(jera(2023,11,2))
# %%
# %%
#(1) 달력 출력 #년도와 달을 입력받아서 출력

#(2) 리스트 컴프리헨션을 이용해서 병렬 저항의 합성저항값을 출력
...
def printCalendar(year, mon) :
    ...
    for w in weeks :
        if w == '일' :
            #빨간색
            print(f'\033[31m{w:^2}\033[0m', end=' ')
        elif w == '토' :
            #파란색
            print(f'\033[34m{w:^2}\033[0m', end=' ')
        else :
            print(f'{w:^2}', end='')
    print()
    for r in range(len(calendar)) :
